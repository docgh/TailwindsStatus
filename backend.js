...
function getCache() {
    if (cacheData && (Date.now() - cacheTimestamp < ((settings.cache_duration_minutes || 5) * 60 * 1000))) {
        return cacheData;
    }
    return null;
}
...
function getRunwayData(data) {
    ...
    if (components.headwind >= 0) {
            runway.student = getColor({ clouds: data.clouds, 
            headwind: components.headwind, 
            crosswind: components.crosswind, 
            gust_headwind: gusts.headwind,
            gust_crosswind: gusts.crosswind,
        ...
        runway.vfr = getColor({ clouds: data.clouds, 
            headwind: components.headwind, 
            crosswind: components.crosswind, 
            gust_headwind: gusts.headwind,
            gust_crosswind: gusts.crosswind,
        ...
        runway.ifr = getColor({ clouds: data.clouds, 
            headwind: components.headwind, 
            crosswind: components.crosswind, 
            gust_headwind: gusts.headwind,
            gust_crosswind: gusts.crosswind,
...
function getColor(data) {
    ...
    if ((Math.max(data.headwind, data.gust_headwind) > data.headwind_max) || 
    (Math.max(unsigned(data.crosswind), unsigned(data.gust_crosswind)) > data.crosswind_max) || (data.cloud_min > ceiling.base) || (data.vis < data.vis_min)) {
        return 'red';
    }
    if ((Math.max(data.headwind, data.gust_headwind) > data.headwind_caution) || 
    (Math.max(unsigned(data.crosswind), unsigned(data.gust_crosswind)) > data.crosswind_caution) || (data.cloud_caution > ceiling.base) || (data.vis < data.vis_caution)) {
        return 'yellow';
    }
    return 'green';
}
...
app.get('/api/weather', async (req, res) => {
  try {
    ...
    const response = await fetch(
      'https://aviationweather.gov/api/data/metar?ids=' + settings.airport + '&hours=0&order=id%2C-obs&sep=true'
    );
