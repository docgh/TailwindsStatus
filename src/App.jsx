...
async function fetchWeatherData() {
  const response = await fetch('http://localhost:5174/api/weather');
...
function getImage(crosswind) {
  ...
  return $('<img>').attr('src', crosswind > 0 ? fromRight : fromLeft)
    .addClass('windDirImg');
  }
...
function Runway(runway, weather) {
  ...
  dataDiv.append($('<p>').text('Headwind: ' + runway.headwind + ' G' + runway.gust_headwind + ' knots'));
  dataDiv.append($('<p>').append($('<span>').append('Crosswind: ').append(getImage(runway.crosswind)).append(disp(runway.crosswind) + ' G' + disp(runway.gust_crosswind) + ' knots')));
  dataDiv.append($('<div>').append(StatusBoxes([ 
  ...
  imageDiv.append($('<img>').attr('src', runwayImg)
  .attr('alt', runwayImg)
  .css('width', '200px')
  .css('height', 'auto')
  .css('transform', 'rotate(' + (runway.runway * 10)+ 'deg)'));
  imageDiv.append($('<img>').attr('src', arrowImg)
  .css('width', '200px')
...
function WeatherDiv(weather) {
  ...
  div.append('<br>');
  div.append($('<span class="wd">').text('Wind Speed: ' + (weather.speed ?? 'N/A') + ' kt'));
  div.append($('<span class="wd">').text('Wind Direction: ' + (weather.dir ?? 'N/A') + '°'));
  div.append($('<span class="wd">').text('Wind Gusts: ' + (weather.gust ?? 'N/A') + ' kt'));
...
function statusDiv(runways) {
  ...
  let student = 'red', vfr = 'red', ifr = 'red';
  runways.forEach((rw) => {
  ...
  return $('<div class="statusDiv">').append(StatusBoxes([ 
...
function StatusBoxes(statuses, size) {
  ...
  const container = $('<div>').addClass('statusBoxesContainer').css({ display: 'flex', gap: '1em', margin: '1em 0' });
...
function App() {
  ...
  async function getWeatherData() {
        ...
        node.append(WeatherDiv(data.weather)).append(statusDiv(data.runways)).append(clear()).append('<hr>');
        if (data.runways && data.runways.length > 0) {
          setRunways(node, data);
        }
  }
  const body = $('<div class="weather-main">');
  body.append('<h1>Tailwinds Weather Status</h1>')
  body.append(node);
